...
- No script yet emits the explicit statistic you need: for each chain, compute `max_spf(false_node)` over all false semiprime-shadow nodes before the true terminal, then correlate that **least-factor frontier** against PGS-visible quantities (`p`, seed offset `s0`, chain deltas/gaps, mod-30 residues of nodes, open density before/after, carrier `d(w)` or GWR score of the chamber winner, chamber state vector, etc.).
