...
The generator outputs PGS-inferred primes. Classical primality methods may be applied afterward as an external validation layer, but they are not part of the generator itself.
...
3. Output Layer
    * Outputs q as a PGS-inferred prime.
...
A number outputted by the generator as the inferred next prime endpoint according to deterministic PGS rules.
...
The generation path must remain free of classical primality testing. The validation path may use classical methods, but only after the generator has output inferred primes.
...
6. The generator outputs q_hat as a PGS-inferred prime.
...
* Outputs inferred primes and inference metadata only.
...
Runs PGS inference first, then validates the outputted list externally.
...
The generator outputs a sequence of records.
...
The number satisfies the active deterministic inference rule set and is outputted by the generator.
...
The outputted inferred prime has been validated after generation by external classical methods.

Level 4: Certified Prime

The outputted inferred prime has a formal primality certificate from an accepted proof system.
...
* Output per-step inference metadata.
* Run classical validation only after the sequence is output.
...
The first outputted inferred prime that is not the actual next prime after its input prime.
...
It outputs inferred primes with complete inference traces.
