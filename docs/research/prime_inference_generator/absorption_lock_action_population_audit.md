...
The audit checks whether a lock's rule-refinement population covers the candidates that the same lock can act on during integration.

The required invariant is:

$$\text{missed action candidates} = 0$$

If a lock can act on a candidate that was not represented in its rule-refinement population, the rule-refinement result is incomplete. If that missed action is wrong or rejects the true next prime, the lock is quarantined as integration logic.
...
| lock | rule-refinement candidates | integration actions | missed actions | missed false absorbers | missed with true next prime unresolved | wrong absorptions | true next prime rejected | status |
|---|---:|---:|---:|---:|---:|---:|---:|---|
...
| lock | surface | rule-refinement candidates | integration actions | missed actions | wrong absorptions | true next prime rejected |
|---|---:|---:|---:|---:|---:|---:|
...
The 005A rule-refinement population covers every absorber that 005A can trigger in integration on these surfaces. This does not approve generator output. It preserves 005A as a narrow candidate-grade lock.
...
The rule-refinement population did not cover this action because the true next prime was unresolved before absorption. That is exactly the mismatch the audit was designed to expose.
