# DCI and the Riemann Hypothesis

The cleanest bridge between the **Divisor Curvature Identity** (DCI) and the
Riemann Hypothesis begins with one ordinary arithmetic fact:

the DCI is built from the divisor count `d(n)`, and the divisor count already
lives inside the zeta function.

In this repository, the DCI is

$$
Z(n) = n^{1 - d(n)/2}
$$

at the fixed traversal rate

$$
v = \frac{e^2}{2},
$$

with curvature load

$$
\kappa(n) = \frac{d(n)\ln n}{e^2}.
$$

That is the coefficient-side arithmetic surface of the project.

The Riemann Hypothesis concerns the zero geometry of the same zeta object on
the analytic side. The shared structure is not a loose resemblance. It can be
written down exactly.

## Exact Dirichlet-Series Bridge

Start with the divisor-count Dirichlet series on the half-plane
\(\operatorname{Re}(s) > 1\):

$$
D(s) = \sum_{n \ge 1} \frac{d(n)}{n^s} = \zeta(s)^2.
$$

Now build the generating series for the DCI curvature load:

$$
K(s) = \sum_{n \ge 1} \frac{\kappa(n)}{n^s}
= \frac{1}{e^2}\sum_{n \ge 1} \frac{d(n)\ln n}{n^s}.
$$

Differentiate \(D(s)\) termwise on the same half-plane:

$$
D'(s)
= -\sum_{n \ge 1} \frac{d(n)\ln n}{n^s}.
$$

So the curvature series is

$$
K(s) = -\frac{1}{e^2}D'(s).
$$

At the fixed traversal rate \(v = e^2/2\), the normalized curvature-to-divisor
ratio is therefore

$$
v \frac{K(s)}{D(s)}
= \frac{e^2}{2}\frac{K(s)}{D(s)}
= -\frac{1}{2}\frac{D'(s)}{D(s)}
= -\frac{\zeta'(s)}{\zeta(s)}.
$$

The classical Euler-product identity then gives

$$
-\frac{\zeta'(s)}{\zeta(s)}
= \sum_{n \ge 1} \frac{\Lambda(n)}{n^s},
$$

where \(\Lambda(n)\) is the von Mangoldt function.

This is the exact arithmetic bridge:

- the DCI begins from `d(n)` and `kappa(n)`,
- their native Dirichlet series recover \(\zeta(s)^2\) and its logarithmic derivative,
- and the fixed-rate DCI normalization lands directly on the classical
  prime-power detector \(-\zeta'/\zeta\).

That bridge is already present before any spectral interpretation is added.

## Proposition

On the half-plane \(\operatorname{Re}(s) > 1\),

$$
D(s) = \zeta(s)^2,
\qquad
K(s) = -\frac{1}{e^2}D'(s),
\qquad
\frac{e^2}{2}\frac{K(s)}{D(s)} = -\frac{\zeta'(s)}{\zeta(s)}
= \sum_{n \ge 1} \frac{\Lambda(n)}{n^s}.
$$

Since \(D(s)=\zeta(s)^2\) on that half-plane, the same identities determine
the meromorphic continuation of the DCI-built objects:

- \(D(s)\) continues as \(\zeta(s)^2\),
- \(K(s)\) continues as \(-D'(s)/e^2\),
- and the normalized ratio continues as \(-\zeta'(s)/\zeta(s)\).

So the DCI does not merely resemble an analytic prime detector.

Its native generating objects reconstruct the standard one exactly.

## Why the Fixed Rate Is Native Here

The role of the repository constant

$$
v = \frac{e^2}{2}
$$

has two exact meanings in this project.

On the integer-field side, it is the fixed traversal rate at which the DCI
collapses to

$$
Z(n) = n^{1-d(n)/2}.
$$

On the Dirichlet-series side, the same scalar is the one that cancels the
factor \(2\) coming from differentiating

$$
D(s)=\zeta(s)^2.
$$

That is,

$$
\frac{e^2}{2}\frac{K(s)}{D(s)}
= \frac{e^2}{2}\left(-\frac{1}{e^2}\frac{D'(s)}{D(s)}\right)
= -\frac{1}{2}\frac{D'(s)}{D(s)}
= -\frac{\zeta'(s)}{\zeta(s)}.
$$

If the curvature normalization were rescaled, this scalar would change with
it. Under the repository's fixed definition
\(\kappa(n)=d(n)\ln n/e^2\), \(v=e^2/2\) is the exact native scalar on both
sides of the bridge.

## Coefficient-Side and Spectral-Side Collapse

The DCI and the Riemann Hypothesis each collapse arithmetic structure onto a
one-dimensional invariant boundary.

For the DCI:

- if \(p\) is prime, then \(d(p)=2\),
- so \(Z(p)=1\) exactly,
- and primes lie on the fixed-point band \(Z = 1.0\).

For the Riemann Hypothesis:

- the nontrivial zeros of \(\zeta(s)\) are conjectured to satisfy
  \(\operatorname{Re}(s)=1/2\),
- so the zero set collapses onto one vertical line in the complex plane.

These are different objects, but they have the same formal shape:

- DCI gives a coefficient-side collapse on the integer field,
- RH gives a spectral-side collapse on the analytic continuation of the
  generating object,
- both single out one invariant locus rather than a diffuse region.

This is the most compact conceptual narrative for the synergy:

the DCI fixes where primes sit in the arithmetic field generated by divisor
structure, while RH fixes where zeta zeros sit in the analytic geometry of the
same underlying generating object.

## RH as Pole Placement of the DCI Ratio

The continued ratio

$$
R(s) = \frac{e^2}{2}\frac{K(s)}{D(s)}
$$

is not a new auxiliary object. It is exactly

$$
R(s) = -\frac{\zeta'(s)}{\zeta(s)}.
$$

That gives one direct reformulation of the RH connection.

- the simple pole at \(s=1\) is the prime-number-theorem pole,
- the trivial zeros of \(\zeta\) become trivial poles of \(R(s)\),
- the nontrivial zeros of \(\zeta\) become nontrivial poles of \(R(s)\).

So the Riemann Hypothesis can be read as a pole-placement statement for the
continued DCI ratio:

all nontrivial poles of
\(\frac{e^2}{2}\frac{K(s)}{D(s)}\) lie on the critical line
\(\operatorname{Re}(s)=1/2\).

This is a precise analytic reading, not a metaphor.

It also clarifies the presentation surface:

- DCI supplies the coefficient-side construction of the ratio,
- meromorphic continuation supplies the global analytic geometry,
- RH constrains the location of the nontrivial poles of that same ratio.

## Why the Gap-Ridge Program Fits Naturally Beside RH

Prime gaps have two boundaries and a composite interior.

The Riemann Hypothesis matters most when prime locations are studied in short
intervals, because zeta-zero geometry governs how sharply prime counts can
fluctuate around their main term.

The DCI gap-ridge program studies the complementary part of that same local
picture:

- once the prime endpoints are fixed,
- which interior composites sit closest to the prime band,
- and what arithmetic structure determines the local raw-`Z` peak.

The current repository already has exact and tested interior structure for that
question:

- the gap-local raw-`Z` maximum is enriched at edge distance `2`,
- the carrier is overwhelmingly `d(n)=4`,
- and on the tested surface the peak agrees exactly with the lexicographic rule
  "smallest `d(n)`, then leftmost".

Those results are documented in:

- [docs/dci_gap_ridge.md](./dci_gap_ridge.md)
- [docs/gap_ridge/raw_composite_z_gap_edge.md](./gap_ridge/raw_composite_z_gap_edge.md)
- [docs/findings/lexicographic_winner_take_all_peak_rule.md](./findings/lexicographic_winner_take_all_peak_rule.md)

This creates a natural division of labor.

On the endpoint side, RH and explicit-formula methods describe how prime
boundaries can fluctuate.

On the interior side, the DCI field ranks the composites that fill the gap
between those boundaries.

That is a genuine structural fit:

- RH organizes prime-gap endpoint behavior through zeta-zero geometry,
- DCI organizes prime-gap interior behavior through divisor-curvature geometry.

## Why the `d(n)=4` Ridge Matters

The prime band is the `d(n)=2` layer.

The first composite layer adjacent to it is the `d(n)=4` layer, including
numbers such as \(pq\) with distinct primes \(p\) and \(q\), and also prime
cubes \(p^3\). For that layer,

$$
d(n)=4
\qquad\Longrightarrow\qquad
Z(n)=n^{-1}.
$$

That layer is not rare decoration in the current repository. It is the
dominant carrier of the gap-local raw-`Z` peak on the tested surface.

The same divisor count belongs naturally to the \(\zeta(s)^2\) side of the
bridge because `d(n)` is exactly the coefficient sequence of that square.

This gives a precise arithmetic reading of the observed ridge:

- primes form the fixed-point `d=2` band,
- the strongest composite layer adjacent to that band is the `d=4` layer,
- and the gap-ridge result says that this first composite layer dominates the
  interior peak geometry.

So the DCI gap ridge is not only a spatial statement about where the interior
peak sits. It is also a statement about which divisor-complexity layer sits
closest to the prime band inside the exact integer field.

## Residue-Class Structure and the L-Function Direction

The current repository also shows that the orientation of the near-edge ridge
depends materially on the left endpoint prime modulo `30`.

That result is documented in
[docs/gap_ridge/residue_mod30_ridge_orientation_note.md](./gap_ridge/residue_mod30_ridge_orientation_note.md)
and
[docs/findings/residue_mod30_ridge_orientation.md](./findings/residue_mod30_ridge_orientation.md).

This matters because congruence classes are already the natural language for
Dirichlet characters and L-functions.

The arithmetic progression direction of prime-distribution theory does not
replace the zeta function. It refines it by splitting the integer line into
residue-conditioned subfamilies.

That means the residue-modulated ridge orientation has a natural next analytic
home:

- the global DCI field sits beside \(\zeta(s)\),
- residue-conditioned DCI subregimes sit beside character-weighted L-series.

The current repository does not need to widen its production path to use that
fact. It is already enough that the same organizing variable appears on both
sides:

- residue class on the arithmetic field side,
- character decomposition on the analytic side.

## What This Suggests Technically

The strongest next research path is not to force an operator story too early.

The stronger immediate move is to treat the DCI as a coefficient-side geometry
whose native generating functions already recover the classical zeta detector.

That yields several concrete targets.

1. Study the DCI divisor series \(D(s)=\zeta(s)^2\) and curvature series
   \(K(s)=-(1/e^2)D'(s)\) as the canonical analytic envelope of the DCI field.
2. Express gap-local DCI observables against short-interval prime statistics,
   especially interior peak location and `d(n)=4` carrier share.
3. Build residue-conditioned versions of those observables and compare them to
   the arithmetic-progression side of prime distribution.
4. Search for a Hilbert-space or transfer-operator formulation only after the
   coefficient-side and residue-conditioned structures are written cleanly.

This order keeps the arithmetic facts in front:

- first identify the exact generating objects,
- then identify the tested local observables,
- then ask whether a spectral realization exists that preserves both.

## Scope

This repository already establishes:

- the exact DCI at \(v = e^2/2\),
- the deterministic production prefilter built from that invariant,
- and the exact raw-DCI gap-ridge observables developed in the committed notes
  and benchmark artifacts.

This note records why those objects sit naturally beside the Riemann Hypothesis.

The exact bridge is the formula

$$
\frac{e^2}{2}\frac{K(s)}{D(s)} = -\frac{\zeta'(s)}{\zeta(s)}.
$$

That identity is enough to make the relationship precise:

- DCI supplies a local integer-field geometry built from divisor structure,
- RH concerns the zero geometry of the analytic detector recovered from that
  same divisor-curvature envelope.

## Anchor Documents

For the exact DCI and production path:

- [docs/dci_prefilter.md](./dci_prefilter.md)
- [technical-note/technical_note.md](../technical-note/technical_note.md)
- [spec/contract.md](../spec/contract.md)
- [src/python/geodesic_prime_rh_bridge/bridge.py](../src/python/geodesic_prime_rh_bridge/bridge.py)

For the exact gap-ridge research surface:

- [docs/dci_gap_ridge.md](./dci_gap_ridge.md)
- [docs/gap_ridge/raw_composite_z_gap_edge.md](./gap_ridge/raw_composite_z_gap_edge.md)
- [docs/gap_ridge/residue_mod30_ridge_orientation_note.md](./gap_ridge/residue_mod30_ridge_orientation_note.md)
- [docs/findings/lexicographic_winner_take_all_peak_rule.md](./findings/lexicographic_winner_take_all_peak_rule.md)
